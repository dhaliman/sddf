...
void init(void)
{
    uint32_t rx_data_size, tx_data_size;
    serial_cli_data_size(microkit_name, &rx_data_size, &tx_data_size);
    serial_queue_init(&rx_queue_handle, rx_queue, rx_data_size, rx_data);
    serial_queue_init(&tx_queue_handle, tx_queue, tx_data_size, tx_data);
