...
static inline uint64_t net_cli_mac_addr_info(char *pd_name)
{
...
static inline void net_virt_mac_addr_info(char *pd_name, uint64_t *macs)
{
...
static inline void net_cli_queue_info(char *pd_name, size_t *rx_queue_size, size_t *tx_queue_size) {
    if (!sddf_strcmp(pd_name, NET_CLI0_NAME)) {
...
static inline void net_copy_queue_info(char *pd_name, size_t *client_queue_size, size_t *virt_queue_size) {
    if (!sddf_strcmp(pd_name, NET_COPY0_NAME)) {
        *client_queue_size = NET_RX_QUEUE_SIZE_CLI0;
        *virt_queue_size = NET_RX_QUEUE_SIZE_COPY0;
    } else if (!sddf_strcmp(pd_name, NET_COPY1_NAME)) {
        *client_queue_size = NET_RX_QUEUE_SIZE_CLI1;
...
static inline void net_virt_queue_info(char *pd_name, net_queue_t *cli0_free, net_queue_t* cli0_active, queue_info_t *ret) {
    if (!sddf_strcmp(pd_name, NET_VIRT_RX_NAME)) {
        ret[0] = (queue_info_t) {.free = cli0_free, .active = cli0_active, .size = NET_RX_QUEUE_SIZE_COPY0};
        ret[1] = (queue_info_t) {.free = (net_queue_t *)((uintptr_t)cli0_free + 2 * NET_DATA_REGION_SIZE),
        ...
        ret[0] = (queue_info_t) {.free = cli0_free, cli0_active, .size = NET_TX_QUEUE_SIZE_CLI0};
        ret[1] = (queue_info_t) {.free = (net_queue_t *)((uintptr_t)cli0_free + 2 * NET_DATA_REGION_SIZE),
                                 .active = (net_queue_t *)((uintptr_t)cli0_active + 2 * NET_DATA_REGION_SIZE),
...
static inline void net_mem_region_info(char *pd_name, uintptr_t *mem_regions, uintptr_t start_region)
{
