                                          ...
                                          serial_info_t ret[SERIAL_NUM_CLIENTS])
{
    if (!sddf_strcmp(pd_name, SERIAL_VIRT_TX_NAME)) {
        ...
        ret[2] = (serial_info_t) {.queue = (serial_queue_t *)((uintptr_t)cli0_queue + 2 * SERIAL_QUEUE_SIZE),
                                .data = cli0_data + SERIAL_TX_DATA_REGION_SIZE_CLI0 + SERIAL_TX_DATA_REGION_SIZE_CLI1 ,
                                .data_size = SERIAL_TX_DATA_REGION_SIZE_CLI2};
    }
}
