...
void init(void)
{
    ...
    serial_virt_queue_init_sys(microkit_name, tx_queue_cli0, tx_data_cli0, queue_info);
    for (int i = 0; i < NUM_SERIAL_CLIENTS; i++) {
        serial_queue_init(&tx_queue_handle_cli[i], queue_info[i].cli_queue, queue_info[i].size, queue_info[i].cli_data);
    }
